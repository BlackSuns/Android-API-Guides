...
内容管理器不是通过意图启动的。它只响应来自[ContentResolver][ContentResolver]的请求。内容解析器处理与内容管理器的所有约定操作，因此组件不需要使用管理器执行操作而是调用[ContentResolver][ContentResolver]对象的方法，这就使得在请求信息的组件和内容管理器之间保留了一个抽象层。
...
+ 可调用[ContentResolver][ContentResolver]的[query()](http://developer.android.com/reference/android/content/ContentProvider.html#query(android.net.Uri, java.lang.String[], java.lang.String, java.lang.String[], java.lang.String))检索内容管理器的数据；

关于使用意图的更多内容请查看[意图和意图过滤器](http://developer.android.com/guide/components/intents-filters.html)文档。关于如何启动指定组件的更多内容请查看这些文档：[Activities](http://developer.android.com/guide/components/activities.html)、[服务](http://developer.android.com/guide/components/services.html)、[广播接收器](http://developer.android.com/reference/android/content/BroadcastReceiver.html)和[内容管理器](http://developer.android.com/guide/topics/providers/content-providers.html)。
